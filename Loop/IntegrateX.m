...
IntegrateX::ferr="Integrating over feynman parameters failed, \!\(\*SubsuperscriptBox[\(\[Integral]\), \(0\), \(1\)]\)dx (1-x\!\(\*SuperscriptBox[\()\), \(a\)]\) \!\(\*SuperscriptBox[\(x\), \(b\)]\) with a or b is integer and \[LessEqual]-1 involved. If the propagators are generated by FourierPX or FourierXP, Set Continue->True in FourierPX or FourierXP can bypass this problem, but the evaluation will be slow, consider to change the way to evaluate the diagram."
...
(*IntegrateX[expr:Except[_List],xx_List/;Length[xx]>1,opts___Rule]:=(-1)^Length[xx](2 Pi)^(Length[xx]D)IntegrateP[expr,xx,opts]*)
...
(* add qdelta to the exponent n1 and n2 in int dx 1/(x-a)^2)^n1 1/(x-n)^2)^n2, to avoid the case the n1 or n2 involve D and the feynman parameters finally yields Integrate[(1-s)^a s^b,{s,0,1}] with a or b is integer and <= -1 *)
fpower=list[[1,2]]+list[[2,2]];
(*If[addqdelta, fpower = fpower+2qdelta];*)
...
(*------ coefficients comes from x^0  \[Rule] -i x^0( p^0 \[Rule] i p^0 ), and integrate over x -----*)
		...
		,
			...
			co1=(-1)^tmp6(-1)^Quotient[4(tmp7+1/2/.D->0),4]I^(Quotient[Mod[4(tmp7+1/2/.D->0),4],2])(-1)^Expand[tmp7-(tmp7/.D->0)+Mod[Mod[4(tmp7+1/2)/.D->0,4],2]/4];
		];
...
(* -----coefficients comes from feynman paramaterization, and integrate over parameter -----*)
		...
		If[MatchQ[{n+p1+D/2 -list[[2,2]], n+p2+D/2 -list[[1,2]]}, ({_,pp_Integer}|{pp_Integer,_})/;pp<=0],
			Message[IntegrateX::ferr];
			Abort[]
		,
		(*If[addqdelta,
			tmp5=tmp5 qGamma[fpower]/(qGamma[list[[1,2]]+ qdelta]qGamma[list[[2,2]]+ qdelta]) qGamma[n+p1+D/2 -list[[2,2]]- qdelta]qGamma[n+p2+D/2 -list[[1,2]]- qdelta]/qGamma[2n+p1+p2+D-fpower]
		,*)
