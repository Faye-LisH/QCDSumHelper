...
IntegrateX::reerr = 
	"Fail to recursively integrate the integral!"
	
IntegrateX::inputerr = 
	"DiracTrace involved but haven't been evaluated!"	
...
If[!FreeQ[tmp,PropagatorDenominator[Momentum[x|x+__|__ -x,___],mass_/;ToString[mass]!="0"]],
	Message[IntegrateX::inputerr];Abort[]];
...
(* avoid mathematica take (-1)^(-1/2 D) as i^D; simplify (-1)^n since there are many term merely differ by a sign*)
...
If[!FreeQ[tmp,nullpo2[aa_]/;Length[aa]>2],Message[IntegrateX::reerr];Abort[]];
