...
TryRecursive::usage =
	"TryRecursive[expr_,momentums_List] is a function to check whether a feynman integral is reducible;
...
Begin["`Private`TryRecursive`"]


	
Options[TryRecursive] = {
	toTFI->True,
...
TryRecursive[expr_,p_Symbol,rules:OptionsPattern[]]:=TryRecursive[expr,{p},rules]



TryRecursive[expr_,mom_List,OptionsPattern[]]:=Block[{numerator,numerator2,tmp,pair2,head,null,count,list,mom1=mom,mom0,tmp2loop,tmp2=1,tmp22,tmp3,tmp4,i,tmprule,mom00,tarcer,
...
tmp=(expr//FCI)/.TraditionalForm->Identity;
...
(***---  expand momentums that involve loop momentum in numerator  ---***)

tmp=Expand[(tmp/.Pair[Momentum[aa_Times,dim___],LorentzIndex[llo_,dim___]]/;Or@@(!FreeQ[aa,#]&/@mom):>Pair[Momentum[aa//Expand,dim],LorentzIndex[llo,dim]])/.
	Pair[Momentum[aa_Plus,dim___],LorentzIndex[llo_,dim___]]/;Or@@(!FreeQ[aa,#]&/@mom):>(Pair[Momentum[#,dim],LorentzIndex[llo,dim]]&/@aa)];


(* if involve more than one term *)
If[ToString[Head[tmp]]=="Plus",
	tmp=TryRecursive[#,mom1,toTFI->totfi,EvaluateTwoLoop->e2loop,ShowSteps->False]&/@(List@@tmp);
	tmp=Flatten[tmp/.{nn_Integer,listt_List,aa_}:>head[nn,listt,aa]]/.head->List

,

(*---------- get the momentum of propagators ----------*)
(* e.g. 1/((c*(p+k))^2)^n -> mom0[p+k, c^2n, n] *)
	...
	(* Expand D-4 or 4 dimensional momenrum to D dimensional momentum, accompany with D-4 or 4 dimensional metirc *)
...
(**--- gather the equivalent momentum, e.g. 2k-q ~ k-q/2; count the factor and exponent, e.g. (5k+5p)^3 => mom0[5k+5p,1,3]=> mom0[k+p,5^(2*3),3] ---**)
...
(* understand the structure of tmp3, e.g. tmp3 = {{{k1,3},{1,0,1,0,0,1,...}},...}, which means:

	tmp:              k1-p, k2-p,  k1,  k2, p-q, k1-k2, q -l

	k1 :   {{{k1,3}, { 1   ,  0  , 1 ,  0,   0,   1   ,   0}},
	k2 :    {{k2,3}, { 0   ,  1  , 0 ,  1,   0,   1   ,   0}},
	p  :    {{p,3} , { 1   ,  1  , 0 ,  0,   0,   1   ,   0}},
	q  :    {{q,2} , { 0   ,  1  , 0 ,  0,   1,   0   ,   1}}}
*)
...
(**-----------------------------------------------**)
		...
		If[OptionValue[Contract]===True,
			...
			{#[[1]],#[[2]],Contract[numerator2 #[[3]],ExpandScalarProduct->False]}&/@DeleteCases[TryRecursive[#,mom]&/@tmp,0|{_,_,0}]
			(* It find that recover the numerator2 before TryRecursive for each term make the evaluation very solw *)
		,
			{#[[1]],#[[2]],numerator2 #[[3]]}&/@DeleteCases[TryRecursive[#,mom]&/@tmp,0|{_,_,0}]
		]
