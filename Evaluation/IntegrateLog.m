 ...
 *)
			...
			,
				...
				,
					If[Xor[FreeQ[su,DirectedInfinity],FreeQ[sl,DirectedInfinity]],
						{liml,limu}={sl,su}/.{{DirectedInfinity[a_],___}:>{a,0},{___,DirectedInfinity[a_]}:>{0,a}}
					,
			...
			If[opt3==={},
				{liml,limu}/.{{0,0}->(tmp/.s->su) -(tmp/.s->sl)(* if none of su and sl are specified *),
								{0,aa_}/;aa=!=0:>(Limit[tmp,s->su,Direction->limu]) -(tmp/.s->sl),
								{aa_,0}/;aa=!=0:> (tmp/.s->su)-(Limit[tmp,s->sl,Direction->limu]),
								{aa_,bb_}/;aa=!=0&&bb=!=0:>(Limit[tmp,s->su,Direction->limu]) -(Limit[tmp,s->sl,Direction->liml])
								}
			,
				{liml,limu}/.{{0,0}->(tmp/.s->su) -(tmp/.s->sl)(* if none of su and sl are specified *),
								{0,aa_}/;aa=!=0:>(Limit[tmp,s->su,Direction->limu,opt3]) -(tmp/.s->sl),
								{aa_,0}/;aa=!=0:> (tmp/.s->su)-(Limit[tmp,s->sl,Direction->limu,opt3]),
								{aa_,bb_}/;aa=!=0&&bb=!=0:>(Limit[tmp,s->su,Direction->limu,opt3]) -(Limit[tmp,s->sl,Direction->liml,opt3])
								}					
			]
			(*
			If[{liml,limu}=={0,0},
					(tmp/.s->su) -(tmp/.s->sl)(* if none of su and sl are specified *)
			...
			*)
		,
			(tmp/.s->su) - (tmp/.s->sl)(* if don't know how to interpret the limu *)
		]
	]
]
...
tmp=tmp//.int[c_]/;!FreeQ[c,log2] :>(c/.a_ log2[t_] :>int[null a]log[t]-int2[int[null a] D[t,x]/t]); (* intergal by part untill no log2 *)
