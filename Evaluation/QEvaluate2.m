...
QEvaluate2::usage = 
"QEvaluate2[expr,p,EpsOrder->0]expand expr to \[Epsilon]-Series "
QEvaluate2::onebyone = "Expand the experssion totally and evaluate the series expansion for each one of them."
...
{tmp,tmp2,tmp3,null0,null,null1,null2,list,list2,nullist,tmplist,log,plus,nlocal=True,ieps,ilog,dimm=OptionValue[D],ldim=4,ldim2=4,ord=OptionValue[EpsOrder],i,j,subtract=OptionValue[Subtract],fact},

tmp=If[OptionValue[SimplifyFlavor]===True,
	(expr/.FlavorDelta[fa_,fb_]:>If[fa===fb,1,0])//FCI
,
	expr//FCI
];
...
(* some times there's warning "Unable to decide whether numeric quantity ... is equal to zero. Assuming it is. " expand tmp can prevent it but cause time consuming  *)
		...
		,
			...
			tmp=Check[Expand[Series[tmp,{Epsilon,0,ord}]],Message[QEvaluate2::onebyone];tmp3=Expand[tmp];Expand[Series[#,{Epsilon,0,ord}]&/@tmp3],{Series::ztest1}]
		];
...
(*--------------------------------------------------------------*)
	...
	(* make every term have (1/Epsilon)^_ , to make the Collect[] can collect terms by EpsOrder; times null1[0,0] to make scalar results can also be collected *)		
			list=null1[0,0]list Epsilon^(-ord-1);
			list=Collect[#,null1[___]]&/@list;		
	(* simplify for each term with same lorentz structure, restore the correct EpsOrder of Epsilon *)
