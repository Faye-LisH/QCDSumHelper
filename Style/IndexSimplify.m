...
IndexSimplify::inderr = "QSSRHelper don't know how to interpret the dummy indices."
...
IndexSimplify[expr_,OptionsPattern[]]:=Block[{tmp,tmp2,tmp3,tmp4,power,warning,nonc,dot,times,null0,null1,
	lindex,dgamma,sigma,dchain,eps,pair,sindex,sunt,suntf,sunf,sund,sundelta,function,lor,tmpindx,indices,indxhead,rule1,rule2,rule3,symm={"null"}},
...
(* now all SUNT appear before DiracGamm, separate color and lorentz space *)
...
,
	...
	(* collect the lorentz tensor in tmp3[] *)
	...
	(* copy the expression for late compare the dummy indices *)
	...
	tmp=(Sort[#]&/@tmp)/.aa_tmp3:>Sort[aa];(* sort for later convience *)

	(* replace the Heads to avoid potential problem *)
	tmp=tmp/.{Eps->eps,DiracGamma->dgamma,DiracSigma->sigma,DiracChain->dchain,Pair->pair,SUNT->sunt,SUNTF->suntf,SUNF->sunf,SUND->sund,SUNDelta->sundelta}/.
	...
	tmp=(tmp/.{lindex->LorentzIndex,sindex->SUNIndex}/.
		{pair->Pair,eps->Eps,dgamma->DiracGamma,sigma->DiracSigma ,dchain->DiracChain,sunt->SUNT,suntf->SUNTF,sunf->SUNF,sund->SUND,sundelta->SUNDelta}/.
	...
	If[OptionValue[QSimplify2]===True,
		tmp=(Plus@@(QSimplify2[Plus@@#,Separate->OptionValue[QSimplify2Rules],SimplifyGamma->OptionValue[SimplifyGamma]]&/@#))&/@tmp;
	,
...
(*--- brutely generate all terms with indices differ by cyclic or reverse ---*)
(**--- if the terms in dot[] differ by cyclic are equivalent ---**)
If[!FreeQ[ToLowerCase[ToString[#]]&/@symmetry,"cyclic"],
	tmp=(#/.bb_dot:>(n=Length[bb];dot2[n,Table[dot@@Join[List@@bb[[j+1;;]],List@@bb[[1;;j]]],{j,0,n-1}]]))&/@tmp;(*generate dot2[the cyclic of dot[...]]*)
		
	tmp=(tmp2={#};
		While[!FreeQ[tmp2,dot2],(* it could be involve many dot2[...] *)
		
			tmp2=Flatten[
				(position=FirstPosition[#,dot2];
				Table[ReplacePart[#,position[[;;-2]]->Extract[#,Join[position[[;;-2]],{2,i(* i th dot[...] in dot2[...] *)}]]],{i,1,Extract[#,Join[position[[;;-2]],{1}(* length dot[...] *)]]}]
				)&/@tmp2]
...
(**--- if the terms in dot[] differ by reverse are equivalent ---**)
...
(* based on the #[[2]] in {_,_,_} in the output of gatherdummy2 *)
...
(* e.g. 
{{c* f[y^u]* g[sigma^uv]* tmp3[q^v], c* f[y^u]* g[sigma^uv], tmp3[q^v]},
 ...} 
-> 
{{c*f[y^u]* g[sigma^uv]]* tmp3[q^v], c*f[y^u]* g[sigma[{sigma,{u,v}}]], tmp3[q^v], {u, {sigma,u}, {sigma,v}, v} },
 ...} *)
(* for DiracSigma[., .], eps[., ., ., .], the position of lorentz index is irrelevant, label the index as {sigma,index_} or {eps,index_} for later procedure *)
...
(* if involve dummy index that appears in more than two place *)
warning=(Times@@Replace[#[[4]], {_,aa_}:>aa,{1}])&@tmp;
If[!FreeQ[warning,Power[aa_,bb_]/;bb>2],Message[IndexSimplify::inderr];Abort[]];



(* e.g. 
{{c* f[y^u]* g[y^u]]* tmp3[q^v], c* f[y^u]* g[y^u], tmp3[q^v], {u,u,v} },
 ...} 
-> 
{{c* f[y^u]* g[y^u]]* tmp3[q^v], f[y^lor]g[y^lor]q^lor, function[f,u]* function[g,u]* function[pair,v], {u,u,v} },
 ...} *)
...
tmp={#[[1]],#[[2]]/.{times->Times,tmp3->Times},#[[3]],
	...
	to undersand it, just note if both are appear in eps, move it to the later; if one appear in eps, move it to another one that not in eps.  
	the propose is that label in this way to avoid to dicsuss the positon of dummy index in eps or DiracSigma *)
...
(* label the nth dummy index in a funciton by nth prime number *)
tmp={#[[1]],#[[2]],(tmp4=#[[3]];
	For[i=1,i<Length[ #[[4]] ]+1,i++,
		...
		,
			...
			(* if the dummy index hve one appear in one of eps of DiracGamma, label the dummy index by the position of another one that not in eps and DiracGamma *)
